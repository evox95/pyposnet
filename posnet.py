...
def crc16(txt: str):
    ...
    hi = 0
    lo = 0
    for s in [ord(c) for c in txt]:
        index = hi ^ s
        hi = lo ^ crc16htab[index]
        lo = crc16ltab[index]
    # print("{:04X}".format((hi << 8) | lo))
    return "{:04X}".format((hi << 8) | lo)


def cmd(ser: serial.Serial, command: str, params=None):
    """
    ...
    c = b'\x02%s\x09#%s\x03' % (
        bytes(cmd_str, encoding='windows-1250'),
        bytes(crc16(cmd_str + '\x09'), encoding='windows-1250')
    )
    ser.write(c)
